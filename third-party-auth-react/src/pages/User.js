...
function User() {
  ...
  const fetchHistory = async (id) => {
    ...
    let history = await axios.get("http://65.20.66.129:5000/history/" + id);
  ...
  const getUser = async (user) => {
    ...
    let userDate = await axios.get("http://65.20.66.129:5000/user/" + user);
  ...
  const updateUser = async () => {
    ...
    let user = await axios.put("http://65.20.66.129:5000/user/" + userId, userDate);
    if (user) {
      setAlertMessage({
